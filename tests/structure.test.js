...
assert.ok(source.includes('data-brand-title="대영포장 WMS/WCS"') && source.includes('data-brand-description="대영포장 WMS/WCS 기획"'), 'Brand text must have one shared source of truth.');
...
assert.ok(app.includes("mainContent.addEventListener('wheel'"), 'Unused main background must forward wheel scrolling to the active content area.');
assert.ok(app.includes("sidebar.addEventListener('wheel'"), 'Non-scrollable sidebar space must forward wheel scrolling to the active content area.');
assert.ok(app.includes("event.target.closest('.content-area')"), 'Wheel forwarding must not interfere with content-area scrolling.');
...
assert.ok(source.includes('class="content-sync-guide"'), 'The content JSON action guide table is missing.');
assert.ok(source.includes('브라우저 화면 → 프로젝트 폴더'), 'The project save direction must be explained in the content JSON guide.');
assert.ok(source.includes('프로젝트 폴더 → 브라우저 화면'), 'The published content restore direction must be explained in the content JSON guide.');
assert.ok(source.includes('site-content.json 파일 → 브라우저 화면'), 'The JSON import direction must be explained in the content JSON guide.');
assert.ok(fs.readFileSync(path.join(root, 'css', 'style.css'), 'utf8').includes('.content-sync-guide'), 'The content JSON action guide table styles are missing.');
...
assert.ok(source.includes('id="sidebarTitle"') && source.includes('대영포장 WMS/WCS'), 'The sidebar title was not updated.');
